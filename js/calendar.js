...
var Calendar = (function(){
	...
	var itemTemp = [
		...
		'	<span class="date_lunar<%=fetvCls%>" title="<%=lunar%>"><%=lunar%></span>',
		'</div>'
	];
	...
	function addEvent(){ //base hammer.js
		...
		$('.last-month').hammer().on('tap',function(event){
			var lastMonth = LunarCalendar.calendar(current.year,current.month-1,true);
			if(current.day > lastMonth.monthDays){
				pageDate(-1,current.year,current.month-1,lastMonth.monthDays);
			}else{
				pageDate(-1,current.year,current.month-1,current.day);
			}
			return false;
		});

		$('.next-month').hammer().on('tap',function(event){
			var nextMonth;
      if (current.month >= 12) {
        // 跳到下一年的1月
        nextMonth = LunarCalendar.calendar(current.year + 1, 1,true);
        pageDate(1,current.year + 1, 1 ,nextMonth.monthDays);
        return;
      } else if (current.month <= 0) {
        // 跳到上一年的12月
        nextMonth = LunarCalendar.calendar(current.year - 1, 12,true);
        pageDate(1,current.year - 1, 12 ,nextMonth.monthDays);
        return ;
      }

      nextMonth = LunarCalendar.calendar(current.year,current.month+1,true);
      if(current.day > nextMonth.monthDays){
				pageDate(1,current.year,current.month+1,nextMonth.monthDays);
			}else{
				pageDate(1,current.year,current.month+1,current.day);
			}
		...
		$('.last-year').hammer().on('tap',function(event){
			var isleapYear = (current.year%100===0 && current.year%400===0 ) || (current.year %100!==0 && current.year%4===0);
			if(isleapYear && current.day > 28){
				pageDate(-1,current.year-1,current.month, 28);
			}else{
				pageDate(-1,current.year-1,current.month,current.day);
			}

			return false;
		});

		$('.next-year').hammer().on('tap',function(event){
